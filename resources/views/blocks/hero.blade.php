<div class="{{ $block->classes }}">
  <div class="w-full mb-4 bg-cover bg-no-repeat bg-center h-screen text-secondary xl:rounded-bl-[291px] flex items-center justify-center bg-[url({{ "'" . $background . "'" }})]">
    <div class="container flex flex-col gap-y-4">
      ...
      <p class="w-full text-xl xl:w-1/2 font-jost">{{ $subtitle }}</p>
      <a class="text-center font-jost text-lg text-white hover:text-primary flex items-center justify-center gap-3 transition-all w-1/6 bg-secondary rounded-[18px] p-7" href="{{ $ctaLink }}" target="_blank">
        {{ $ctaLabel }}
